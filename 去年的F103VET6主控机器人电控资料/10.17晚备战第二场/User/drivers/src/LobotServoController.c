...
* 日期：20160806
* LSC系列舵机控制板二次开发示例
	moveServo(1, 2000, 1000); //1舵机号，2000舵机位置，1000时间
	delay_ms(3000);
	moveServos(2, 800, 2,1200,9,2300); //2个舵机，800时间，2号舵机1200位置，9号舵机2300位置
	delay_ms(2000);
	runActionGroup(1, 1); //1号动作组运行一次
	delay_ms(1500);
  stopActionGroup();  //停止运行动作组
  setActionGroupSpeed(1, 200);//一号动作组速度200%
*******************************************************************************/
#include "stm32f10x.h"
...
void moveServo(uint8_t servoID, uint16_t Position, uint16_t Time)
{
	...
	Usart_SendArray( LSC_USART, LobotTxBuf, 10);
}
...
void moveServosByArray(LobotServo servos[], uint8_t Num, uint16_t Time)
{
	...
	Usart_SendArray( LSC_USART, LobotTxBuf, LobotTxBuf[2] + 2);
}
...
void runActionGroup(uint8_t numOfAction, uint16_t Times)
{
	...
	Usart_SendArray( LSC_USART, LobotTxBuf, 7);
}
...
void stopActionGroup(void)
{
	...
	Usart_SendArray( LSC_USART, LobotTxBuf, 4);
}
/*********************************************************************************
