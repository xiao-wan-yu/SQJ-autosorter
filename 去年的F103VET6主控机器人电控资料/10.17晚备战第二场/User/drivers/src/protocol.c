...
struct prot_frame_parser_t                                                     //环形缓冲区
{
    uint8_t *recv_ptr;                                                         //接收数据指针
    uint16_t r_oft;                                                            //读偏移
    uint16_t w_oft;                                                            //写偏移
    uint16_t frame_len;                                                        //帧长度
    uint16_t found_frame_head;                                                 //帧头标志位
};
  ...
  while(1)
  {
    ...
    switch (cmd_type)
    {
      ...
      case SET_P_I_D_CMD:
      {
        ...
        //set_p_i_d(&pid2_v,p_temp, i_temp, d_temp);    // 设置 P I D
      }
      break;
...
void set_computer_value(uint8_t cmd, uint8_t ch, void *data, uint8_t num)
{
  ...
  Usart_SendArray(USART1, (uint8_t *)&set_packet, sizeof(set_packet));    // 发送数据头
  Usart_SendArray(USART1, (uint8_t *)data, num);                          // 发送参数
  Usart_SendArray(USART1, (uint8_t *)&sum, sizeof(sum));                  // 发送校验和
}
